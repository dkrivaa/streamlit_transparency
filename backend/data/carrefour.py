...
class CarrefourParent(SupermarketChain):
    ...
    async def get_files(cls, client: httpx.AsyncClient | None = None) -> dict:
        ...
        try:
            ...
            # Extract JSON string inside JSON.parse(`...`) containing all files
            match = re.search(r'const\s+files\s*=\s*JSON\.parse\(`(.*?)`\)', html, re.S)
            if not match:
    ...
    async def promo_blacklist(cls) -> set[str]:
        ...
        return {'11366992', '80135876'}  # When blacklist the format should be: {"4305214"}
